...
var SECTIONS = [
    {
        ...
        generateHTML: function () {
            ...
            var plotArea = '<div class="text-center">' +
                '<button type="button" class="btn btn-primary btn-sm graphs-plot-button" onclick="generateMixedGraphs_()">' +
            ...
            var genToolbar = function (view) {
                ...
                view.sensors.forEach(function (sensorId) {
                    try {
                        ...
                        if (_sensor_) {
                            ...
                            sensorsSL += '<li><input type="checkbox" id="' + _selectId_ + '"><label for="' + _selectId_ + '" name="NAME" value="VALUE">' + _sensor_.name + '</label></li>';
                        }
                ...
                setTimeout(function(){
                    $("[id='"+view._id + "-average-checkbox']").bootstrapSwitch();
            ...
            if (_this_.views.length > 1) {
                ...
                tabLinks += '<li class="active"><a data-toggle="tab" href="#' + _this_.views[0]._id + '">'+ _this_.views[0].name + '</a></li>';
                tabs += '<div id="' + _this_.views[0]._id + '" class="tab-pane fade in active">' + genToolbar(_this_.views[0]) + plotArea + '</div>';

                _this_.views.slice(1, _this_.views.length).forEach(function (view) {

                    tabLinks += '<li><a data-toggle="tab" href="#' + view._id + '">'+ view.name + '</a></li>';
                    tabs += '<div id="' + view._id + '" class="tab-pane fade">' + genToolbar(view) + plotArea + '</div>';
                });
